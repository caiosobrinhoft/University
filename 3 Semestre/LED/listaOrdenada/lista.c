...
#define TRUE 1     //define tipo boleano e nao existe em C
#define FALSE 0
#define boolean int
...
{
   //Retorna true (1): se cheia, false (0): caso contrario
   return (L->nelem == MAX);
}
...
void Definir(lista *L)
{
   ...
   para cada nova lista antes de qualquer outra operacao.*/
   L->nelem = 0;
   L->A[0].chave = 0;
}
...
{   
   /*Retorna true, se x ocorre na posicao p. Se x ocorre mais de
   uma vez, retorna a posicao da primeira ocorrencia. Se x nao
   ocorre, retorna false. Para listas NAO-ORDENADAS*/
...
{
   /*So e ativada apos a busca ter retornado a posicao p
   do elemento a ser removido - Nro de Mov = (nelem e p)*/
...
int Tamanho(lista *L)
{
   //Retorna o tamanho da lista. Se L e vazia retorna 0
...
boolean Inserir_ord(tipo_elem x, lista *L)
{
   ...
   { 
	  ...
	  return Inserir_posic(x,i,L); //i=nelem (insere na ultima posicao)
   }
}
		  ...
		  if (L->A[i].chave >= x)
              if (L->A[i].chave == x)
			  {
                  *p = i;
                  ...
                  return FALSE; //Encontrou maior, entao para
          else
              i++;
   }
...
{
   /*Remocao dada a chave. Retorna true, se removeu, ou
   false, c.c.*/
